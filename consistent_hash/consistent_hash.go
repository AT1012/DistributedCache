...
//TODO this is wrong. we need to find the largest key less than the given key
//Modify it accordingly. Couldn't find any ceil function.
func (c *Consistent) searchPrevious(key uint32) (i int) {
	f := func(x int) bool {
		return c.sortedHashes[x] < key
	}
	i = sort.Search(len(c.sortedHashes), f)
...
// GetN returns the N closest distinct elements to the name input in the circle.
func (c *Consistent) GetPreviousN(name string, n int) ([]string, error) {
	c.RLock()
	defer c.RUnlock()
