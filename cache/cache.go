...
func (c *cache) Set(k string, x interface{}, d time.Duration, isReplica bool) {
	// "Inlining" of set
	var e int64
...
func (c *cache) Add(k string, x interface{}, d time.Duration, isReplica bool) error {
	c.mu.Lock()
	_, found := c.get(k)
	if found {
		c.mu.Unlock()
...
func (c *cache) Replace(k string, x interface{}, d time.Duration, isReplica bool) error {
	c.mu.Lock()
	_, found := c.get(k)
	if !found {
		c.mu.Unlock()
...
func (c *cache) GetNotExpiredItems() map[string]Item {
	...
	for key, item := range c.items {
		if !item.Expired() && !item.IsReplica {
			retMap[key] = item
		}
	}
	c.mu.RUnlock()
